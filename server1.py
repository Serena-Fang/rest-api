...
def load_topic_data():
    ...
    print("loaded data with ", len(topic_df_data), "records")
...
def get_topics():
    global topic_df
    response = topic_df.loc[:, ~topic_df.columns.isin(restricted_column_list)].to_json(orient="records", default_handler=str)
    return Response(response)
